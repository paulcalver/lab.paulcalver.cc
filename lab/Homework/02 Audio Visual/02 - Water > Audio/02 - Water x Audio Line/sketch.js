...
let numNotes = 7; // Number of note zones - now matches detection sections
...
let detectionSections = 7; // Reduced to 7 for cleaner look with larger circles
...
function preload() {
   //Load 7 unique sounds - one for each detection circle
   tones = [
      loadSound('assets/flutter_07.mp3'), // Highest note (top of line)
      loadSound('assets/flutter_06.mp3'),
      loadSound('assets/flutter_05.mp3'),
      loadSound('assets/flutter_04.mp3'),
      loadSound('assets/flutter_03.mp3'),
      loadSound('assets/flutter_02.mp3'),
      loadSound('assets/flutter_01.mp3')  // Lowest note (bottom of line)
    ];
}
  ...
  for (let sectionIndex = 0; sectionIndex < detectionSections; sectionIndex++) {
    ...
    if (detectionCounts[sectionIndex] > 0) {
      ...
      if (avgSectionBrightness > sparkleThreshold) {
        ...
        const noteIndex = sectionIndex; // Direct mapping - each circle has its own note
...
function triggerSparkleHit(noteIndex, yPosition, brightness) {
  ...
  if (audioStarted && !audioPaused && !isMuted && tones[noteIndex] && !tones[noteIndex].isPlaying()) {
...
function drawSamplingUI(videoX, videoY, displayW, displayH) {
  ...
  stroke(255, 255, 255); // Bright red
  strokeWeight(4); // Thin line
  line(canvasBarX, lineTop, canvasBarX, lineBottom); // Fill entire line
  noStroke();

  // Update detection area to match the full line
  const detectionTop = lineTop - barTop; // Offset from video top
  const detectionSectionHeight = lineHeight / detectionSections;
  for (let i = 0; i < sparkleDetections.length; i++) {
    const sparkleIntensity = sparkleDetections[i];
    if (sparkleIntensity > 5) { // Show even faint sparkles
      // Position circles to fill the entire line from top to bottom
      const sparkleY = lineTop + (i * detectionSectionHeight) + (detectionSectionHeight / 2);

      // Base circle size and growth when reacting
      const baseSize = 20; // Increased from 12 to 20
      const maxGrowth = 15; // Increased from 8 to 15
      const growthAmount = (sparkleIntensity / 255) * maxGrowth;
      const circleSize = baseSize + growthAmount;

      // White circle with intensity-based alpha
      fill(255, 255, 255, sparkleIntensity * 1 + 50); // White with minimum 50 alpha
      noStroke();
      ellipse(canvasBarX, sparkleY, circleSize, circleSize);
    }
  }
