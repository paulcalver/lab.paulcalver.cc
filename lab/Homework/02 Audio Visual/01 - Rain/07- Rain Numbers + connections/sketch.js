...
function drawIntensityNumber(alpha = 255, intensity = 0.5) {
  push();
  fill(0, alpha);
    ...
    for (let j = 0; j < cols; j++) {
      ...
      if (directImpact) {
        ...
        if (elapsed < animationDuration) {
          ...
          // Draw intensity number using transform
          push();
          translate(bx, by);
          ...
          // Draw number showing audio intensity
  ...
  for (let i = 0; i < activeDrops.length; i++) {
    for (let j = i + 1; j < activeDrops.length; j++) {
      ...
      if (intensity1Rounded === intensity2Rounded) {
        ...
        let lineAlpha = min(drop1.alpha, drop2.alpha) * 0.5; // Dimmer than the drops
        
        // Draw dashed connection line
        push();
        stroke(0, lineAlpha); // Gray color with matching alpha
        strokeWeight(1);
        
        // Set up dashed line pattern
        drawingContext.setLineDash([3, 3]); // 5 pixels on, 3 pixels off
