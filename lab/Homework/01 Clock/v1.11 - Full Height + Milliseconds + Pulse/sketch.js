...
let SECOND_PULSE_COLOR; // <-- make this global
...
function setup() {
  ...
  // Pulse colour for the active second
  SECOND_PULSE_COLOR = color(0, 0, 0, 255);
  // colours
...
// ---------- SECONDS (full height, pulsing active block in black) ----------
function drawSecondRow(now, x, w, h) {
  ...
  for (let i = 0; i < SECOND_COUNT; i++) {
    ...
    if (i === S) {
      // Pulse the active second from black → steady colour over the second
      const tRel = constrain(curSec - i, 0, 1);
      const eased = easeOutSine(tRel);
      col = lerpColor(SECOND_PULSE_COLOR, blockColor, eased);
      op = OPACITY_MAX;
    } else if (i < S) {
      // trailing fade for previous seconds (unchanged)
