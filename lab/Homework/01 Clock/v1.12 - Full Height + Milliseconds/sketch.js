...
const HOUR_TRAIL_UNITS = 10; // how many past hours keep a visible trail
...
// ---------- HOURS (full height, mirrors minute/second fade + trailing window) ----------
function drawHourRow(now, x, w, h) {
  ...
  for (let i = 0; i < HOUR_COUNT; i++) {
    ...
    if (i === H) {
      // active hour fades in over the hour
      const tRel = constrain(curHours - i, 0, 1);
      op = lerp(OPACITY_BASE, OPACITY_MAX, tRel);
    } else if (i < H) {
      // trailing fade for previous hours (limited to HOUR_TRAIL_UNITS)
      const age = max(0, (curHours - i) - 1); // how many hours since this hour finished
      if (age <= HOUR_TRAIL_UNITS) {
        const k = constrain(age / HOUR_TRAIL_UNITS, 0, 1);
