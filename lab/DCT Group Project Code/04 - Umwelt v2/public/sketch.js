...
let minTimeBetweenDrops = 0; // Very fast drops for multiple simultaneous drops

// Sound file mappings to weather variables and motion control
const soundMappings = {
    'ES_Gentle_In_Water_On Wood_Drip_Quiet.mp3': { weather: 'dewpoint24', min: -10, max: 20, motionAxis: 'x' },
    'ES_Heavy Rain_Rainfall 02.mp3': { weather: 'rainProb24', min: 60, max: 100, motionAxis: 'y' },
    'ES_Heavy Rain_Rainfall 03.mp3': { weather: 'rain24', min: 5, max: 20, motionAxis: 'z' },
    'ES_Light Rain_Forest Brazil_Increasing Intensity_04.mp3': { weather: 'humidity24', min: 60, max: 100, motionAxis: 'x' },
    'ES_Light Rain_OnCement_Light Wind.mp3': { weather: 'wind24', min: 10, max: 40, motionAxis: 'y' },
    'ES_Light Rain_Rainfall_01.mp3': { weather: 'rainProb24', min: 20, max: 60, motionAxis: 'z' },
...
function setup() {
    ...
    // Setup EQ filters for each sound layer
    Object.keys(soundLayers).forEach(filename => {
        let sound = soundLayers[filename];
        
        // Create filter chain: lowShelf -> midPeak -> highShelf
        let lowShelf = new p5.Filter('lowshelf');
        let midPeak = new p5.Filter('peaking');
        let highShelf = new p5.Filter('highshelf');
        
        // Set base frequencies
        lowShelf.freq(200);   // Below 200 Hz
        highShelf.freq(6000); // Above 6 kHz
        midPeak.freq(1200);   // Around 1.2 kHz
        midPeak.res(1.0);     // Q / bandwidth
        
        // Connect: sound -> lowShelf -> midPeak -> highShelf -> output
        sound.disconnect();
        sound.connect(lowShelf);
        lowShelf.connect(midPeak);
        midPeak.connect(highShelf);
        // highShelf connects to master output automatically
        
        // Store filters
        soundFilters[filename] = {
            lowShelf: lowShelf,
            midPeak: midPeak,
            highShelf: highShelf
        };
    ...
    // Update EQ based on motion 10x per second
    setInterval(updateEQFromMotion, 100);
}
...
function updateSoundVolumes() {
    Object.keys(soundMappings).forEach(filename => {
        ...
        if (sound) {
            ...
            // Detect if phone is flat (screen up): x≈0, y≈0, z≈-9.8
            let isFlatScreenUp = motionX > -0.1 && motionX < 0.1 && motionY > -0.1 && motionY < 0.1 && abs(motionZ + 9.8) < 0.5;
            ...
            // Mute all sounds when phone is flat (screen up)
            if (isFlatScreenUp) {
                finalVolume = 0;
            }
...
// Update EQ filters based on motion data
function updateEQFromMotion() {
    // Normalize motion values with more sensitive ranges for obvious effect
    let motionXNorm = map(abs(motionX), 0, 10, 0, 1, true); // More sensitive (0-10 instead of 0-20)
    let motionYNorm = map(abs(motionY), 0, 10, 0, 1, true); // More sensitive
    let motionZNorm = map(abs(motionZ + 9.8), 0, 10, 0, 1, true); // More sensitive (adjusted for gravity)
    
    // Apply EQ to all sound layers
    Object.keys(soundFilters).forEach(filename => {
        let filters = soundFilters[filename];
        
        // motionX controls low frequencies - MUCH MORE DRAMATIC (-12 to +12 dB)
        let lowGain = map(motionXNorm, 0, 1, -12, 12);
        filters.lowShelf.gain(lowGain);
        
        // motionY controls high frequencies - MUCH MORE DRAMATIC (-12 to +12 dB)
        let highGain = map(motionYNorm, 0, 1, -12, 12);
        filters.highShelf.gain(highGain);
        
        // motionZ controls mid scoop/boost - MORE DRAMATIC (-15 to +3 dB)
        // More Z motion = boost mids, less motion = scoop mids heavily
        let midGain = map(motionZNorm, 0, 1, -15, 3);
        filters.midPeak.gain(midGain);
    });
}
...
let gridSize = height * 1;
  ...
  if (anySoundPlaying) {
    ...
    swColour = map(smoothedAudioLevel, 0.08, 0.3, 200, 0, true); // 0.5 = max expected audio level
  ...
  if (anySoundPlaying) {
    ...
    if (normalizedLevel > audioThreshold && millis() - lastTriggerTime > minTimeBetweenDrops) {
      // Number of drops based on audio intensity (1-20 drops)
    ...
    for (let j = 0; j < cols; j++) {
      ...
      if (directImpact) {
        ...
        if (elapsed < animationDuration) {
          ...
          let intensityScale = map(directImpact.intensity || 0.5, 0.05, 0.35, 0.5, 5.0, true);
  ...
  for (let i = 0; i < activeDrops.length; i++) {
    for (let j = i + 1; j < activeDrops.length; j++) {
      ...
      if (intensity1Rounded === intensity2Rounded) {
        ...
        // Convert calculatedAlpha from 0-255 range to 0-1 range for HSB
        let normalizedAlpha1 = drop1.alpha / 255;
        let normalizedAlpha2 = drop2.alpha / 255;
        let lineAlpha = min(normalizedAlpha1, normalizedAlpha2) * 0.5; // 0.5 = initial alpha multiplier (adjust this)
