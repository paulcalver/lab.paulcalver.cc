...
function setup() {
  ...
  {
    lookUpTable.push(int(random(maxQueueSize)));
  }
  ...
  if(frameQueue.length >= maxQueueSize){
    ...
    for(let x = 0; x < width; x+=imgWidth){
      for(let y = 0; y < height; y+=imgHeight){
        ...
        //get our randomised index values
        let lookUpIndex = lookUpTable[imgIndex];
        //draw our frame that was stored in our frame buffer
        image(frameQueue[lookUpIndex],x,y,imgWidth,imgHeight);
        imgIndex++;
